...
Write a function named sayHello,
 that sends the message 'Hello from the back-end' when a user hits the `/hello` route.
...
const oddValues = (arr) => arr.filter((n) => {
    if (n % 2 == 1) {
        return n;
    }
});
...
, given two arrays as input,
 uses filter to return an array of all 
...
const notInFirstArray = (forbiddenValues, arr) => {
    // Solution code here...
    return arr.filter( (item) => !forbiddenValues.includes(item)); //includes returns true or false if it is truthy it will return val
