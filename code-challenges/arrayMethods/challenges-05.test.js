...
Write a function named templateWithJQuery that uses jQuery to get the html template
 from the DOM, copy the contents, fill it with the Star Wars People, and append it to the DOM.
------------------------------------------------------------------------------------------------ */
let starWarsPeople = [
  {
    "name": "Luke Skywalker",
...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(item => {
    let newEl =  temp.first().clone();
    newEl.find('h2').text(`${item.name}`);
    newEl.find('h3').text(`${item.height}`);
    newEl.find('p').text(`${item.eye_color}`);
    main.append(newEl);
  })
}
...
You are making a grocery list for 
ingredients needed in the gruffalo crumble recipe, below.
 Rather than taking the entire recipe, you only want a list of the item names.

Write a function named listFoods that takes 
in the recipe and returns a new array of the food items without 
...
const listFoods = (recipe) => {
  ...
  // Solution code here...
    ...
    result.push(item.slice(item.indexOf(' ',3)).slice(1));
    })
