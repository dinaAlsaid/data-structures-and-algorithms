...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.status(200).send('My name is dina alsaid, people often call me dana or dania or dima, i am sad about that, because my name is easy peasy lemon squeezy');
  });
  app.get('/favoritefoods', (req, res) => {
    res.status(200).send('dark Chocolate, grapes, unicorn tears, fairy-dusted gold-leafed gourmet magical-mushrooms');
  });
  app.get('*', (req, res) => {
...
const count = (target, input) => {
  // Solution code here...
  let reducedArr = input.reduce((a, arr) => a.concat(arr), [])
  let retVal = reducedArr.reduce((a, item) => {
    if (item == target) { a += 1; }
    return a;
  }, 0);
  return retVal;
